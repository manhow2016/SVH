...
export function registerFileRoutes(app: FastifyInstance, deps: FileRouteDeps): void {
  ...
  app.get<{ Params: { id: string }; Querystring: { path?: string } }>(
    ...
    async (req) => {
      await deps.workspaceService.getOwned(req.params.id, req.user!.userId);
      return deps.workspaceService.listFiles(req.params.id, req.query.path ?? ".");
    },
  );
