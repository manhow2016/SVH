 ...
 * 两个调用方都满足，但**理由不同**：
 * - 详情抽屉：表单态初值就是服务端返回的完整 metadata，之后只由 `MetadataForm` 的
 *   `onChange` 写回 —— 货真价实的完整态；
 * - 新建对话框：`current` 其实是**稀疏态**（初值 `{}`，只有被碰过的键才进表单态），
 *   它安全**只因为 `initial` 也是 `{}`**：每个字段都落进规则 3（`before === undefined`），
 *   不会凭空提交 `null`。「传 `{}` 就满足契约」是错的读法。
 * 将来若有调用方只想传一份增量，或给新建场景传一份**非空** initial，请先改这里。
 *
 * 三条规则，缺一不可：
 * 1. 值与初始值相同      → 不提交
