...
**问题**：服务端只把 **Agent 轮次**的载荷落成会话消息，Worker 产出的卡没有任何
落消息路径。后果是刷新页面后任务面板显示「已完成」，对话流里却找不到那张卡。

**修法**：Worker 在任务成功后追加一条 `result_card` 会话消息
（`appendSessionMessage`，`packages/database/src/sessions.ts`）—— 这是会话消息的
**唯一写入入口**，API 的 `appendMessage` 端口也委托给它，避免「kind 收敛」
「payload 的 JSON 断言」这类细节在两处慢慢分叉。消息同时写入 `taskId` 列，
指明产出它的任务。

只写**成功、有卡片、且挂在会话上**的任务：失败说明走任务面板，没有会话的任务
（`POST /api/tasks` 建的）没有可追加的对象。落库失败也不会让已成功的任务变成失败 ——
业务结果已经写进 `agent_tasks`、媒体已在资产库，这里只记日志。

**回捞保留**，作为本次修复之前产生的会话的兜底：`load()` 在拉完历史消息后，按
`GET /api/tasks?sessionId=...&status=success` 取回终态任务，串行拉详情、
把 `output.card` 补进对话流末尾，与实时链路共用 `resultCardDone` /
`resultCardLoading` 去重集合。

去重判据是**消息行的 `taskId` 列**，不是 `payload.taskId`：载荷由各技能自己构造，
`asset.create` 之类的卡压根不带这个字段（第一版按载荷写，被用例当场逮住）。
`load()` 会先把历史里已落库的卡记账，再回捞 —— 否则同一个任务会被补出第二张卡，
刷新一次多一张。回捞的边界不变：

- 窗口是最近 50 个成功任务（`BACKFILL_TASK_LIMIT`），更早的历史不补
  （新会话不再依赖它）；
...
14. **前端侧的真实限制（Phase 5B 交付时确认）**：
    ...
    - ✅ **结果卡不落会话消息**（已修）：此前结果卡只活在**前端内存**里 ——
      界面收到 `task.status` 后自行拉任务详情、在本地拼一条消息，刷新即消失，
      只能靠「回捞最近 50 个成功任务」兜底，超出窗口的卡就永远不见了，
      而界面仍宣称历史完整。

      现在由 **Worker** 在任务成功后把卡片落成一条 `result_card` 会话消息
      （`appendSessionMessage`，同时写入消息行的 `taskId` 列）。
      「刷新后还在」因此是数据库保证的，而不是界面尽力而为的。

      落库的失败**不会**让已经成功的任务变成失败（业务结果已经写进
      `agent_tasks`、媒体已在资产库）：只记日志，实时事件与回捞都还在。

      **回捞保留**，但加了去重：`load()` 会先把历史里已落库的卡按消息行的
      `taskId` 记进去重集合，再回捞。判据用**消息行的列**而不是
      `payload.taskId` —— 载荷由各技能自己构造，`asset.create` 的卡压根不带
      这个字段（第一版按载荷写，被用例当场逮住）。回捞对新会话是冗余的，
      但对本次修复**之前**产生的会话仍是唯一的卡片来源。
    ...
    - **未配置模型时工作台不提示去配置**：`model_providers` 为空时后端的
      ...
      的提示条与空状态主操作。
    - ✅ **没有根级错误边界**（已修）：此前错误边界只有**逐条消息**的
      `MessageBoundary`，它挡得住「一条坏消息」，挡不住渲染期之外的异常
      （路由层、页面组件、Provider 自身）—— React 19 在这种情况下会卸载整棵
      根树，用户看到的是**一片白屏**，没有文案也没有可点的东西。

      现在 `AppErrorBoundary` 包在**最外层**（连 `ToastProvider` 一起覆盖，
      提供者抛错同样会白屏）。降级页复用 `ErrorState`：说清发生了什么、
      附上原始错误信息、给出两条下一步，另有「回到项目列表」的链接。
      「重试」只重置边界状态重挂子树，**不整页刷新** —— 多数渲染异常来自
      一次性的坏数据，重挂就能过去，而刷新会丢掉用户的会话位置与输入内容。
    - ✅ **会话历史一次最多 200 条**（已修）：详情端点现在多取一条来判断
      「光标之前还有没有」，在响应里返回 `hasMore` —— 不让前端看「是不是正好
      取满一页」，因为条数等于 limit 时两种情况的界面行为相反，猜不出确定答案。
      界面据此显示「加载更早的消息」，按**当前最早那条的 `createdAt`** 作游标
      往前取一页并**前插**到对话流顶部；到头后入口消失。
      前插会补偿滚动位置（把新增高度加回 `scrollTop`），否则用户眼前的消息会跳走。
      游标是 `createdAt` 的严格小于，理论上不重复；仍按 `id` 去重一次 ——
      同毫秒多条消息会让边界不稳，而重复渲染比丢消息更烦人。
      真机实测：8 条消息按 2 条一页翻 4 页，`hasMore` 依次 true/true/true/false，
      收齐顺序与原顺序一致、无重复。
    - ✅ **结果卡的媒体加载失败没有与「生成失败」区分**（已修）：存储不可用、
      文件被清理、链接过期时，`<img>` 只渲染成破图、`<video>` 停在无法播放的状态，
      界面既不说明「生成成功了、只是这份媒体打不开」，也不给下一步 ——
      用户会把它读成「这次生成失败了」，**白花一次生成去重跑**。

      现在媒体项单独成组件并接住 `onError`，把该格子换成可读说明 + **原始地址**
      （排查的人第一眼要看的就是它指向哪里）。

      **措辞刻意是「打不开」而不是「取不回来」**：`error` 同时覆盖
      ① 取不回来（404 / 网络 / 被拦）与 ② 取回来了但用不了（解不了码、格式不支持），
      从事件本身分不出是哪种。真机上就撞到过 ② —— 桩服务把一段文本标成
      `video/mp4` 返回（HTTP 200、528 字节完整），`<video>` 照样报错。
      第一版文案写的是「这份文件现在取不回来」，是被真机探针推翻的。
      所以文案只断言确定的事，把两种可能都列出来，不替用户下结论。
      ...
      **夹具限制（非代码问题）**：桩服务的 `/stub/video.mp4` 返回的是一段文本、
      ...
      `/stub/image.png` 是真的 1×1 PNG，图片卡正常。
...
18. ✅ **测试会调用开发者本机配置的模型 Provider**（已修）：
    ...
    **顺带更正一处我先写错的判断**：我一度把「测试强制 Mock」记成
    「三个适配器在 CI 里从不真正发 HTTP 请求」——**这是错的**。
    `packages/model/test/providers.test.ts` 起的是一个**真实的本地 HTTP 服务器**
    （`server.listen(0, '127.0.0.1')`，真实 TCP 套接字），适配器指向
    `http://127.0.0.1:<port>`，`fetch` 走真实网络栈；文件开头就写着
    「不用 mock 掉 fetch：适配器最容易出错的地方恰恰是报文形状」。
    覆盖量：OpenAI 兼容 9 例 / Anthropic 5 例 / Gemini 7 例，外加错误映射 8 例、
    连通性检查 4 例、取消信号传导 1 例。
