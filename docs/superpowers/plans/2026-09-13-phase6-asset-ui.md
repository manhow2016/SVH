...
`README.md` 的三处（行号是实施时的锚点，先 grep 确认再改）：

1. 第 16 行的状态句：`本仓库处于 **V0.1 · Phase 0 ~ Phase 5B 已完成** 状态。`
   → 改成 `Phase 0 ~ Phase 6 已完成`
2. 阶段表（第 27–29 行附近）：`| Phase 6 | Asset System 交互与 @资产 | ⬜ 待开始 |`
   → `| ✅ 完成 |`
3. 正文里若有「Phase 6 待开始」一类的收尾句，一并改掉。

`docs/ARCHITECTURE.md`：

- **新增 `## 6.11 Phase 6 交付：资产库前端`**，插在既有 `## 6.10 Phase 5B 交付：前端结构（Agent UI）`
  那一节结束、`## 7. 本阶段交付边界` 之前（grep `^## 6.10` 与 `^## 7\.` 定位）。
- **更新 `## 9. 已知限制`**（文件末尾附近，grep `^## 9\.` 定位），把本阶段登记的四项 +
  本计划新登记的一项追加进去。
- `## 7. 本阶段交付边界` 里若写着「不做资产库」之类的话，改成已完成。
