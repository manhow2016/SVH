...
Run:
```bash
pnpm --filter @svh/database migrate:dev -- --create-only --name add_storyboard_and_timeline
```
...
Run: `pnpm --filter @svh/database migrate:dev && pnpm --filter @svh/database generate`
