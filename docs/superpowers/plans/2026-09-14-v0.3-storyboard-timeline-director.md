...
describe('导演动作仓储', () => {
  ...
  it('即便调用方试图传 requiresConfirmation 也不生效', async () => {
    const action = await createAction({
      ...
      actor: 'user',
      type: 'delete_shot',
      payload: { targets: [{ type: 'shot', id: 'shot_9' }], changes: {} },
  ...
  it('确认是幂等的：重复确认不改变 confirmedAt', async () => {
    const action = await createAction({
      ...
      actor: 'agent',
      type: 'delete_asset',
      payload: { targets: [{ type: 'asset', id: 'asset_1' }], changes: {} },
    });
  ...
  it('拒绝后不能再被确认', async () => {
    const action = await createAction({
      ...
      actor: 'user',
      type: 'delete_shot',
      payload: { targets: [{ type: 'shot', id: 'shot_7' }], changes: {} },
    });
  ...
  it('执行态写入带 CAS：已执行的动作不能再标记失败', async () => {
    const action = await createAction({
      ...
      actor: 'agent',
      type: 'delete_shot',
      payload: { targets: [{ type: 'shot', id: 'shot_8' }], changes: {} },
    });
...
export async function confirmAction(id: string): Promise<DirectorActionRow> {
  return transition(id, ['awaiting_confirmation'], 'approved', { confirmedAt: new Date() });
}

export async function rejectAction(id: string, reason?: string): Promise<DirectorActionRow> {
  return transition(id, ['awaiting_confirmation', 'failed'], 'rejected', {
