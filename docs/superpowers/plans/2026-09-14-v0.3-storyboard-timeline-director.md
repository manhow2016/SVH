...
## 0. 收尾修订（2026-09-15，与最终实现的对账单）

下面各 Task 里的代码块是**开工前**写的规格，不是最终实现的镜像。凡与仓库冲突，
以仓库为准。本轮按实测同步过的地方：

1. **domain 容差用例（Task 3）**：数据从 `2.0000001`（终点之后）改为 `1.9999999`
   （终点之内）。旧数据是空转断言 —— 实测把 `OVERLAP_EPSILON` 改成 0 后它仍然变绿
   （`1 failed | 14 passed`）。新数据配上同一变异则与「0.1 + 0.2」那条双双变红。
2. **迁移护栏（Task 5）**：破坏性语句禁令的标题限定为「第 4 个迁移」；
   新增一条「第 5 个迁移只做那一件事：重建 `assetId` 外键」的语句形状断言
   （该文件现为 6 个用例）。原因：四条禁令的正则一条都不命中 `DROP CONSTRAINT`，
   直接套用是假通过 —— 两次变异对照（退回 `SET NULL`、追加 `DELETE FROM`）
   都恰好让这一条变红。
3. **domain 秒数 schema（Task 3 / Task 6）**：读模型的两份手抄
   `readStartSecondsSchema` / `readDurationSecondsSchema` 已收敛为唯一的
   `nonNegativeFiniteSecondsSchema`，`clipStartSecondsSchema` 是它的别名。
   摘掉这一处 `.finite()` 会让 6 条用例同时变红（写路径 3 + 读模型 2 + 导演动作 1）。
4. **各步的 `Expected: PASS（N 个用例）` 已按收尾实测更新**：
   domain `storyboard` 8、domain `timeline` 15、database `migration-scope` 6、
   database `storyboard` 14、database `timeline` 26、database `director-actions` 18。
5. **不回填的部分**：计划里嵌入的 database 测试代码块没有包含修复波新增的用例
   （Infinity 探针、asset-only 正例、级联回归 ①②③④ 等）。这些以仓库文件为准。

约定本身见 `docs/ARCHITECTURE.md` §10。

## Global Constraints

- **时长单位统一为秒**（浮点，毫秒精度取整 `Math.round(x * 1000) / 1000`），复用 `durationSecondsSchema`；全链路只此一种单位。
- **既有表 0 列改动**：迁移中**不得出现任何针对既有表的 `ALTER TABLE`**。本片两条迁移
  允许出现的语句只有四类：新表的 `CREATE TABLE`、新表的索引、新表上的手写 `CHECK`、
  以及**新表上**外键的重建（`DROP CONSTRAINT` + `ADD CONSTRAINT`，第 5 条迁移为把
  `timeline_clips.assetId` 从 `SET NULL` 改成 `CASCADE` 而写 —— 它与「恰好一个来源」
  的 CHECK 互斥，见 ARCHITECTURE §9 第 23 条）。
  这条承诺由 `packages/database/test/migration-scope.test.ts` 逐表断言（Task 5），
  第 5 条迁移另由「语句形状」那条断言单独钉住。
...
describe('时间线领域契约', () => {
  ...
  // 反空转（收尾时修订）：起点必须**探进**上一段终点之内（1.9999999 < 2）。
  // 原先这里写的是 `2.0000001`（终点之后）—— 那种数据在 `OVERLAP_EPSILON` 改成 0
  // 时同样通过，是一条怎么都不会红的空转断言。两条容差用例的变异对照输出见
  // ARCHITECTURE §10.1 第 4 条。
...
export const timelineClipSchema = z
  .object({
    ...
    // 读模型的数值同样必须**有限**：带外写入的 Infinity 会让总时长与 EDL 全长
    // 全线污染，而 `nonnegative()` 放行 Infinity（Ruling 25，与写路径同一条理由）
...
export const timelineSchema = z
  .object({
    contentId: idSchema,
    // 总时长是派生值，但同样要过 `.finite()`：它由片段的起点 + 时长算出（Ruling 25）
...
const clipChangeSchema = z
  .object({
    ...
    id: idSchema,
    // **复用** timeline.ts 导出的 `clipStartSecondsSchema`（非负且有限），不要手抄：
    // 同形状再写一遍就会漏掉 `.finite()`，而 `JSON.parse('{"startSeconds":1e999}')`
    // 得到的 `Infinity` 能过 payload 校验、落成 approved，再由 JSONB 静默写成 null
    // —— 终审 Important 1 与 Ruling 10 / 25 的同一个洞
...
describe('迁移范围', () => {
  ...
  // 标题里的范围限定不是客套（收尾时修订）：这条禁令只读第 4 个迁移的文件。
  // 第 5 个迁移本身就是 `DROP CONSTRAINT` + `ADD CONSTRAINT`（重建 assetId 外键），
  // 它对既有表的承诺由上面那条「写入目标白名单（覆盖两个迁移）」承担。
  // 见 ARCHITECTURE §10.2。
...
model TimelineClip {
  ...
  assetId String?
  /// Cascade 而非 SetNull：SET NULL 会把 asset-only 片段变成「无来源」，与下面那条
  /// 「恰好一个来源」CHECK 互斥（删资产会整笔 23514 失败）。修正随第 5 个迁移
  /// `timeline_clip_asset_cascade` 落地 —— Ruling 28
...
> **必测（规范 §7.2 的集成链，别被后面的用例拆散掉）**：除上面的仓储用例之外，还要有一条
> 自成一体的集成链 —— 建 12 个镜头 → `ensureDefaultTracks` → 在 video 轨放 12 个 clip
> （引用镜头）→ **完全打乱后 reorder** → 断言 `index` 连续**且 clip 未错位**（重排前后逐条
> 比对 `(clipId, shotId, startSeconds)`，把重排实现成「删旧行再建新行」会让片段随级联消失）
> → 删第 5 个镜头 → 断言其 clip 级联消失、其余 `index` 重排连续。另需一条「不变量 2 的 DB
> 半边」：绕过仓储直接 `prisma.storyboardShot.create` 写重复 `(contentId, index)`，断言
> `@@unique` 以 **P2002** 拒绝。
 ...
 * 这三条写函数不能直接吃裸 number：`JSON.parse('{"a":1e999}')` 在 JS 里得到
 * `Infinity`，而 Fastify 默认就用 `JSON.parse` 解析请求体 —— 一旦接上路由，
 * `startSeconds: 1e999` 会直接写进 float8 列，让总时长与 EDL 全线变成 `Infinity`。
 * 因此 `createClip` / `updateClip` / `moveClip` 一律**先 parse 再碰数据库**：
 * 校验失败时连事务都不会开，更不会留下半成品。入参类型直接用领域包导出的
 * `CreateClipInput` / `UpdateClipInput` / `MoveClipInput`，不再本地手抄一份形状。
 */
import {
  assertNoOverlap,
  createClipSchema,
...
Run: `pnpm --filter @svh/database exec vitest run test/timeline.test.ts`
Expected: PASS（26 个用例，其中 2 条（双来源 / 无来源）证明 CHECK 在数据库层生效）
