 ...
 * 时间线仓储
 *
 * ── 默认轨道：顺序调用幂等，真并发不保证收敛 ──
 * 界面首次进入时间线时应当看到三条空轨，而「首次」在调用侧是不可靠的
 * （两个标签页同时打开就会各调一次）。`ensureDefaultTracks` 以 kind 为判据
 * 补齐缺失项，因此**顺序**重复调用返回同一批行、不会多建一套。
 *
 * 但真并发下它只是「大概率收敛」：两个事务可能都读到「还没有 video 轨」，
 * 随后都去插入 —— 此时由 `@@unique([contentId, order])` 仲裁，落败的那个
 * 调用方会拿到 P2002（唯一约束冲突），而不是静默收敛成同一批行。当前的
 * 调用场景是页面初始化（同一浏览器标签页串行），故按 brief 不做重试；
 * 若将来出现并发初始化，这里应改为捕获 P2002 后重读一次，或改用
